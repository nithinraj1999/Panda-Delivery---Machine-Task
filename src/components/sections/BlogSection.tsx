...
const blogs = [
  {
    ...
    description:
      "Journey across continents to uncover some of the most inspiring architectural wonders.",
  ...
  },
  {
    ...
    description:
      "Innovative design and eco-conscious principles to shape a more environmentally.",
  ...
  },
  {
    ...
    description:
      "Old buildings are transformed into vibrant spaces while preserving their historical charm.",
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-20">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6">
          <div className="flex flex-col gap-4">
            <button className="flex items-center justify-center gap-2 rounded-full bg-[#FCF3EA] text-black font-semibold px-6 py-3 text-sm w-max">
              <svg
                width="16"
                height="17"
                viewBox="0 0 16 17"
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-5xl font-semibold text-[#003164]">
              See Our Latest Blogs!
            </h2>
          </div>

          <button className="flex items-center gap-2 border border-gray-300 px-5 py-2 rounded-md text-sm font-medium cursor-pointer w-max">
            View All →
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {blogs.map((blog) => (
            <div
              key={blog.id}
              className="rounded-lg overflow-hidden transition bg-white shadow-sm"
            >
              <ImageWithFallback
                src={blog.image}
                alt={blog.title}
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-[#0C0D13] mt-2 font-normal">
                  {blog.description}
                </p>
                <button className="mt-4 text-sm font-semibold text-[#0C0D13] border-b border-[#0C0D13]">
                  READ MORE
                </button>
              </div>
            </div>
